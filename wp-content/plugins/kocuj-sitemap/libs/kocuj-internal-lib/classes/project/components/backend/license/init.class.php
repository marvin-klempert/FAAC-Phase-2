...
class Init extends \KocujIL\V12a\Classes\ComponentInitObject {
	...
	public function __construct($projectObj) {
		...
		// set errors
		$this->errors = array (
				\KocujIL\V12a\Enums\Project\Components\Backend\License\ExceptionCode::LICENSE_FILE_DOES_NOT_EXIST => 'License file does not exist' 
		);
	}
	...
	public function actionsAndFilters() {
		...
		if ((current_user_can ( 'manage_network' )) || (current_user_can ( 'manage_options' ))) {
			$this->getComponent ( 'actions-filters-helper' )->addActionWhenNeeded ( 'wp_ajax_' . $this->getComponent ( 'project-helper' )->getPrefix () . '__license_display', \KocujIL\V12a\Enums\ProjectCategory::BACKEND, 'license', 'ajax', 'actionAjaxDisplay' );
		}
	}
}
