  ...
  public function vimeography_pro_shortcode_atts($result, $pairs, $atts) {
    if ( isset( $atts['id'] ) && intval( $atts['id'] ) != 0) {
      ...
      if (
        ...
        ! empty( $_GET['vimeography_gallery'] ) &&
        absint( $_GET['vimeography_gallery'] ) === absint( $atts['id'] )
      ) {
        $result['containing_uri'] = '/videos/' . absint( $_GET['vimeography_video'] );
        $result['cache'] = 0;
        $result['remote_per_page'] = $result['per_page'];
      }
