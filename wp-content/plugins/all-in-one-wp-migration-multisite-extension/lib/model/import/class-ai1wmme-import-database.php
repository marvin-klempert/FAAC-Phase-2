...
class Ai1wmme_Import_Database {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1wm_Status::info( sprintf( __( 'Restoring multisite database...<br />%d%% complete', AI1WMME_PLUGIN_NAME ), $progress ) );
		...
		foreach ( $blogs as $blog ) {
			...
			foreach ( $home_urls as $home_url ) {
				...
				if ( ! in_array( sprintf( "'%s'", trim( ai1wm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) ), $old_replace_raw_values ) ) {
					...
					$new_replace_raw_values[] = sprintf( "'%s'", get_blog_option( $blog['New']['BlogID'], 'upload_path' ) );
				}
		...
		// Get ms files rewriting
		$ms_files_rewriting = get_site_option( AI1WM_MS_FILES_REWRITING );
		...
		if ( empty( $multisite['Network'] ) ) {
			...
			$new_table_prefixes[] = ai1wm_table_prefix( 'mainsite' );
			...
			foreach ( $blogs as $blog ) {
				if ( ai1wm_main_site( $blog['Old']['BlogID'] ) === true ) {
					foreach ( $wpdb->global_tables as $table_name ) {
						$old_table_prefixes[] = ai1wm_servmask_prefix() . $table_name;
						$new_table_prefixes[] = ai1wm_table_prefix( 'mainsite' ) . $table_name;
					}
				}
			}
		...
		if ( isset( $config['Plugin']['Version'] ) && ( $version = $config['Plugin']['Version'] ) ) {
			if ( $version !== 'develop' && version_compare( $version, '4.10', '<' ) ) {
				...
				foreach ( $blogs as $blog ) {
					$include_table_prefixes[] = ai1wm_table_prefix( $blog['New']['BlogID'] );
				}

				// Set include table prefixes
				if ( $include_table_prefixes ) {
					$mysql->set_include_table_prefixes( $include_table_prefixes );
					$mysql->flush();
				}
			}
		}
		...
		$mysql->set_atomic_tables( array( ai1wm_table_prefix() . 'options', ai1wm_table_prefix() . 'blogs' ) );
		...
		// Activate plugins
		ai1wm_activate_sitewide_plugins( ai1wm_active_servmask_plugins() );
